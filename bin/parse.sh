...
NICKS=('TuxOtaku' 'derjur' 'dr_summer' 'chelsea_' 'vladTO' 'Turdburg' 'apow' 'countryHick')
for n in "${NICKS[@]}"; do
...
# special handling of tyler's many nicks
NICKS=('azend|vps' 'azend')
echo '' > dict/tmp.log
for n in "${NICKS[@]}"; do
  grep "b33r_time.${n}:" log/**/b33r* | sed "s/^log.*\/b33r_time.*${n}://" >> dict/tmp.log
  grep "b33r_time.${n}:" log/b33r* | sed "s/^log.*\/b33r_time.*${n}://" >> dict/tmp.log
done
cat dict/tmp.log | sort -u > dict/q.azend.log

# special handling for the bee
NICKS=('beeee' 'beeeee' 'beeeee_' 'beeeee__' 'beeeee___' 'beeeee____')
echo '' > dict/tmp.log
for n in "${NICKS[@]}"; do
  grep "b33r_time.${n}:" log/**/b33r* | sed "s/^log.*\/b33r_time.*${n}://" >> dict/tmp.log
  grep "b33r_time.${n}:" log/b33r* | sed "s/^log.*\/b33r_time.*${n}://" >> dict/tmp.log
